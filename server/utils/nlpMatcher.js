...
//       const sbertScore = await getSBERTSimilarity(
//         resumeText,
//         jobDescription
//       );
...
// async function getSBERTSimilarity(resumeText, jobText) {
//   try {
...
//   } catch (error) {
//       console.error("SBERT error:", error.message);
//       return 0;
//   }
// }
...
class NLPMatcher {
  ...
  async calculateAdvancedMatch(resumeText, jobDescription) {
    try {
      ...
      const finalScore = (skillsMatch * 0.4) + (stringSim * 0.1) + (sbertScore * 0.5);
      ...
      return {
        ...
        stringSimilarity: Math.round(stringSim * 100),
        skillsMatch: Math.round(skillsMatch * 100),
        semanticSimilarity: Math.round(sbertScore * 100),
        matchedTerms: [],
        missingSkills: this.getMissingSkills(resumeTerms, jobTerms),
        analysis: this.generateAnalysis(finalScore)
      };
